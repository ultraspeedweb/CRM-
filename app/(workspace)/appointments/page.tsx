...
export default async function AppointmentsPage({ searchParams }: Props) {
  ...
  const [appointments, leads] = await Promise.all([
    supabase.from("appointments").select("id,title,starts_at,ends_at,status,location,cancellation_reason,leads(full_name)").eq("organization_id", organizationId).order("starts_at"),
    supabase.from("leads").select("id,full_name").eq("organization_id", organizationId).order("full_name"),
  ]);

  return <main className="content"><PageHeader title="المواعيد" subtitle="جدول المعاينات والاجتماعات مع حالة تشغيلية واضحة حتى الإغلاق." />
    ...
    <article className="panel grow"><div className="panel-head"><div><h2>الجدول</h2><p>Europe / Istanbul</p></div><CalendarDays size={20} /></div><div className="appointment-list">{(appointments.data ?? []).map((item) => { const lead = Array.isArray(item.leads) ? item.leads[0] : item.leads; const date = new Date(item.starts_at); const availableStatuses = nextStatuses[item.status] ?? []; return <div className="appointment-row" key={item.id}><div className="date-tile"><b>{new Intl.DateTimeFormat("ar", { day: "numeric", timeZone: "Europe/Istanbul" }).format(date)}</b><span>{new Intl.DateTimeFormat("ar", { month: "short", timeZone: "Europe/Istanbul" }).format(date)}</span></div><div><strong>{item.title}</strong><span>{lead?.full_name ?? "عميل"} · {item.location ?? "عن بُعد"} · {statusLabels[item.status] ?? item.status}</span>{item.cancellation_reason && <small>{item.cancellation_reason}</small>}</div><time><Clock3 size={15} /> {new Intl.DateTimeFormat("ar", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Istanbul" }).format(date)}</time>{availableStatuses.length > 0 && <form className="stack-form" action={updateAppointmentStatus}><input type="hidden" name="appointmentId" value={item.id}/><select name="status" required defaultValue=""><option value="" disabled>الحالة التالية</option>{availableStatuses.map((status) => <option key={status} value={status}>{statusLabels[status]}</option>)}</select><input name="cancellationReason" maxLength={500} placeholder="سبب الإلغاء مطلوب عند الإلغاء"/><button className="mini-action" type="submit">تحديث</button></form>}</div>})}{!appointments.data?.length && <div className="empty-state"><CalendarDays size={30} /><strong>لا توجد مواعيد</strong><span>اختر عميلًا وأضف أول موعد.</span></div>}</div></article></section>
  </main>;
}

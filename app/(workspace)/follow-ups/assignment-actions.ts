...
export async function assignFollowUp(formData: FormData) {
  ...
  if (!UUID_RE.test(followUpId) || !UUID_RE.test(requestedAssignee)) {
    redirect(destination("error", "تعذر تعيين المتابعة"));
  }

  const canAssignOthers = MANAGER_ROLES.has(membership.role);
  if (!canAssignOthers && requestedAssignee !== userId) {
    redirect(destination("error", "ليس لديك صلاحية لتعيين المهمة لموظف آخر"));
  }
  ...
  if (memberError || !targetMember) {
    redirect(destination("error", "الموظف المحدد غير متاح في هذه المؤسسة"));
  }

  const { data: updated, error } = await supabase
    .from("follow_ups")
    .update({ assigned_to: requestedAssignee })
    .eq("organization_id", organizationId)
    .eq("id", followUpId)
  ...
  if (error || !updated) {
    redirect(destination("error", "تعذر تحديث مسؤول المتابعة"));
  }

  await supabase.from("audit_logs").insert({
    organization_id: organizationId,
    actor_user_id: userId,
    action: "follow_up.assigned",
    entity_type: "follow_up",
    entity_id: followUpId,
    metadata: { assigned_to: requestedAssignee },
  });

...
export async function escalateFollowUp(formData: FormData) {
  ...
  const { data: escalation, error } = await supabase.from("follow_up_escalations").insert({ organization_id: organizationId, follow_up_id: followUpId, escalated_to: escalatedTo, escalated_by: userId, reason }).select("id").single();
  if (error || !escalation) redirect(destination("error", "المتابعة مصعّدة بالفعل أو تعذر إنشاء التصعيد"));
  await supabase.from("audit_logs").insert({ organization_id: organizationId, actor_user_id: userId, action: "follow_up.escalated", entity_type: "follow_up", entity_id: followUpId, metadata: { escalation_id: escalation.id, escalated_to: escalatedTo, reason } });
...
export async function resolveFollowUpEscalation(formData: FormData) {
  ...
  const { data: resolved, error } = await supabase.from("follow_up_escalations").update({ status: "resolved", resolved_by: userId, resolved_at: resolvedAt, updated_at: resolvedAt }).eq("organization_id", organizationId).eq("id", escalationId).eq("status", "open").select("id,follow_up_id").maybeSingle();
  if (error || !resolved) redirect(destination("error", "تعذر إغلاق التصعيد"));
  await supabase.from("audit_logs").insert({ organization_id: organizationId, actor_user_id: userId, action: "follow_up.escalation_resolved", entity_type: "follow_up", entity_id: resolved.follow_up_id, metadata: { escalation_id: resolved.id } });
  revalidatePath("/follow-ups"); revalidatePath("/dashboard");

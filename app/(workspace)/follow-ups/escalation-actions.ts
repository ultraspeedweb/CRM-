...
export async function resolveFollowUpEscalation(formData: FormData) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const escalationDb = supabase as any;
  const { data: resolved, error } = await escalationDb.from("follow_up_escalations").update({ status: "resolved", resolved_by: userId, resolved_at: resolvedAt, updated_at: resolvedAt }).eq("organization_id", organizationId).eq("id", escalationId).eq("status", "open").select("id,follow_up_id").maybeSingle();
